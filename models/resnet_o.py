...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.num_features = outf.shape[1]
...
class Attention3(nn.Module):
    def __init__(self, dim, heads = 8, dropout = 0.1):
        ...
        self.to_qk = nn.Linear(dim, dim, bias = True) # Wq,Wk,Wv for each vector, thats why *3
...
class ViT2ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, avg_pool=4, dim = 512, num_tokens = 512, mlp_dim = 128, heads = 1, depth = 1, emb_dropout = 0.1, dropout= 0.1):
        super(ViT2ResNet, self).__init__()
        ...
        self.transformer = LocalAttention(512, 512, window_size=self.avg_pool)
...
def ResNet18E(num_classes = 10, avg_pool = 4):
    return ResNetEnc(BasicBlock, [2,2,2,2], num_classes, avg_pool)
...
class VTVGG(nn.Module):
    ...
    def __init__(self, features, classes):
        ...
        self.transformer = LocalAttention(512, 512, window_size=4, heads=24)
    ...
    def forward(self, x, mask = None):
        ...
        x, attn = self.transformer(x) #main game
